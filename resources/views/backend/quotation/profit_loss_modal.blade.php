                ...
                <div class="row invoice-info">
                    <div class="col-sm-4 invoice-col">
                        ...
                        <address style="font-size:12px;font-weight:bold; color:#732323;background-color:#999999c9;">
                            ...
                            <span style="padding:2px;">Total Purchase : 
                                <strong style="font-size:14px;">
                                {{  number_format($saleFinal->getQuotationTotalPurchaseAmountFromSaleDetail() , 2,'.','')}}
                                </strong>
                            </span> <br/>
                            <span style="padding:2px;padding-bottom:5px;">Total Profit/Loss : 
                                <strong style="font-size:14px;">
                                    {{ number_format($saleFinal->quotationTotalSaleAmount() - $saleFinal->getQuotationTotalPurchaseAmountFromSaleDetail() , 2,'.','')}}
                                </strong>
                            </span>
                        </address>
                    </div>
                    ...
                    <div class="col-sm-4 invoice-col">
                        ...
                        <b>Date:</b> {{date('d-m-Y h:i:s a',strtotime($saleFinal->sale_date))}}<br />
                            ...
                            <b> Invoice Amount :</b> {{$saleFinal->quotationTotalSaleAmount()}}<br/>
                            <b> Paid Amount : </b> {{$saleFinal->quotationTotalPaidAmount()}} <br/>
                            <b>  Due Amount : </b>{{$saleFinal->quotationTotalDueAmount()}}  <br/>


                        <b>Payment Status:</b>
                            @if($saleFinal->totalPaidAmount() > 0)
                                <span>
                                    @if($saleFinal->quotationTotalSaleAmount() == $saleFinal->quotationTotalPaidAmount())
                                        <span class="badge badge-primary"> Paid </span>

                                    @elseif($saleFinal->quotationTotalSaleAmount() > 0 && $saleFinal->quotationTotalSaleAmount()  < $saleFinal->quotationTotalPaidAmount())
                                            <small class="badge badge-warning"> Over</small><span class="badge badge-primary"> Paid </span>

                                    @elseif($saleFinal->quotationTotalSaleAmount() > 0 && $saleFinal->quotationTotalSaleAmount()  > $saleFinal->quotationTotalPaidAmount())
                                        <span class="badge badge-danger">Due</span>

                                    @elseif($saleFinal->quotationTotalSaleAmount() < 0)
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody >
                                    @php
                                        ...
                                        $totalProfitLoss    = 0;
                                    @endphp
                                    @foreach ($saleFinal->quotationSaleDetails??$saleFinal->quotationSaleDetails as $item)
                                        <tr>
                                ...
                                <tr style="background-color:green;color:white;">
                                    ...
                                    <td>{{ number_format($totalProfitLoss,2,'.','')}} <small>(Less : {{ $saleFinal->quotationDiscountAmountOfSaleFinal() }})</small> </td>
                                </tr>
                                <tfooter>
                            </table>
                        </div>
