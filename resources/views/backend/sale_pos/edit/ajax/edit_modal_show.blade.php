            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="exampleModalLabel">
                        ...
                        <span style="mergin-right:20px;">Update Cart Edit Sale</span>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </h4>
                </div>
                <form action="{{route('sale.editSaleUpdateCartFromModal')}}" method="POST" id="editSaleUpdateCartFromModal"> {{--id="addToCartFromProductShortDetailModal"--}}
                    @csrf
                                ...
                                <table class="table table-bordered table-striped">
                                    <tr>
                                        <td style="width:45%;">
                                            <div class="form-group" style="background-color:rgb(241, 238, 238);padding:0px 5px 6px">
                                                <label>Sale Type</label>
                                                    <select id="" name="sale_type_id" class="cr_sale_type_class form-control">
                                                        <option {{$sale_type_id == 1 ?'selected':''}} value="1">Regular</option>
                                                        <option {{$sale_type_id == 2 ?'selected':''}} value="2">Offer</option>
                                                    </select>
                                            ...
                                            <div class="form-group" style="background-color:rgb(241, 238, 238);padding:0px 5px 6px">
                                                ...
                                                <select id="" name="sale_from_stock_id" class="cr_sale_from_stock_class form-control" data-class="cr_sale_from_stock_class">
                                                    ...
                                                    <option {{ $sale_from_stock_id == $stock->id ?'selected':''}} value="{{$stock->id}}">{{$stock->name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <!---------------------------------------->
                                        ...
                                        <td style="width:30%;">
                                            <div class="form-group" style="background-color:rgb(241, 238, 238);padding:0px 5px 6px">
                                                ...
                                                <select id="" name="sale_unit_id" data-class="cr_sale_unit_class" class="cr_sale_unit_id_class form-control">
                                                    ...
                                                    <option  {{$unit->id == $sale_unit_id ?'selected':''}} value="{{ $unit->id }}">{{ $unit->short_name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="form-group" >
                                                <label class="btn btn-primary">
                                                    <input name="price"  value="{{number_format($product->default_selling_price,2,'.', '')}}"  type="radio" class="cr_priceClass cr_defalutSalePrice cr_defalutSalePriceClass colored-blue">
                                        ...
                                        <td style="width:25%;">
                                            <div class="form-group" style="background-color:rgb(241, 238, 238);padding:0px 5px 6px">
                                                <label>Price</label>
                                                <input name="sale_price" value="{{$sale_price}}" class="form-control cr_sallingPrice" type="number" step="any" placeholder="Price">
                                            </div>
                                            <div class="form-group" style="background-color:rgb(241, 238, 238);padding:0px 5px 6px">
                                                <label>Quantity Amount (Pcs)</label>
                                                <input name="quantity" class="cr_current_quantity_class form-control cr_quantity"  type="number" value="{{$quantity}}" step="any" placeholder="Quantity Amount" data-class="cr_current_quantity_class">
                                            ...
                                            <div class="form-group" style="background-color:rgb(241, 238, 238);padding:0px 5px 6px">
                                                <label>
                                                    ...
                                                    <label style="cursor: pointer;margin-right:5px;">
                                                        <input name="discountType" {{$discountType == "percentage" ? 'checked' :''}} value="percentage" type="radio" class="colored-blue cr_discountTypeClass" style="font-size:12px;cursor: pointer">
                                                        <span class="text" style="font-size:12px;">Percentage (%)</span>
                                                    </label>
                                                    <label style="cursor: pointer">
                                                        <input name="discountType"  {{$discountType == "fixed" ? 'checked' :'' }} value="fixed" type="radio" class="colored-blue cr_discountTypeClass" style="font-size:12px;cursor: pointer">
                                                        <span class="text" style="font-size:12px;">Fixed</span>
                                                    </label>
                                                </label>
                                                <input class="form-control cr_discountValue" name="discountValue" value="{{$discountValue}}" type="number" step="any" placeholder="Only Numeric Value">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" disabled class="cr_addToCart_button btn btn-info">Update Cart</button>
                    </div>
                </form>
            </div>
        </div>
