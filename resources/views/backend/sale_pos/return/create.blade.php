@extends('home')
@section('title',' Return Sale Invoice')
@section('content')
<!-- Page Content -->
            ...
            <div class="col-xs-12 col-md-12">
                <div class="widget">
                    <div class="widget-header bg-info">
                        ...
                        Inovice No : {{$sale_final->order_no}} (Sale Return)
                        </span>
                    ...
                    <form action="{{route('sale.returnSaleStore')}}" method="POST">
                        @csrf
                        <input type="hidden" name="sale_final_id" value="{{ $sale_final->id }}" />
                        <input type="hidden" name="customer_id" value="{{ $sale_final->customer_id }}" />
                        <input type="hidden" name="business_location_id" value="{{ $sale_final->business_location_id }}" />
                        <div class="widget-body" style="background-color: #fff;">
                            <div class="row" style="margin: 30px 0;">
                                <div class="col-md-4">
                                    <label>Invoice No:</label>
                                    <input type="text" name="order_no" readonly value="{{$sale_final->order_no}}" class="form-control"/> <br/>
                                    <label>Sale Date:</label>
                                    <input type="text" readonly value="{{$sale_final->sale_date}}" class="form-control"/>
                                </div>
                                <div class="col-md-4">
                                <label>Customer Name:</label>
                                    <input type="text" readonly value="{{$sale_final->customers?$sale_final->customers->name:'Walking Customer'}}" class="form-control"/> <br/>
                                        ...
                                        @foreach ($cartName as $key => $item)
                                            <tr>
                                                <td>
                                                    ...
                                                    <input type="hidden" name="return_unit_price[]" value="{{ $item['sale_price'] }}" />
                                                    <input type="hidden" name="return_unit_id[]" value="{{ $item['sale_unit_id'] }}" />
                                                    <input type="hidden" name="sale_quantity[]" value="{{ $item['quantity'] }}" />
                                                    <input type="hidden" name="return_stock_id[]" value="{{ $item['sale_from_stock_id'] }}" />
                                                </td>
                                                <td style="width:40%">
                                                    {{$item['name']}}
                                                </td>
                                                ...
                                                <td>
                                                    {{$item['sale_price']}}
                                                    <input type="hidden" value="{{$item['sale_price']}}" id="sale_unit_id_{{ $item['productVari_id'] }}"
                                                </td>
                                                <td>
                                                    {{$item['quantity']}}
                                                    <input type="hidden" value="{{$item['quantity']}}" id="sale_quantity_id_{{ $item['productVari_id'] }}"
                                                </td>
                                                <td style="width:15%">
                                                    <input type="text" value="0" name="return_quantity[]" class="return_quantity_class form-control" id="return_quantity_id_{{ $item['productVari_id'] }}" data-sale_details_id="{{ $item['productVari_id'] }}"/>
                                                </td>
                                                <td>
                                                    <strong id="return_amount_id_{{ $item['productVari_id'] }}" class="get_subtotal_class">0.0</strong>
                                                </td>
                                            </tr>
                                            @php
                                            $i++;
...
<!-- /Page Content -->
    @push('js')
        ...
        <!-----for Ajax handeling----->
        <script src="{{ asset('public') }}/custom_js/backend/sale_return/create.js?v=1"></script>
